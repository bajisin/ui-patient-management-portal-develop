...
import { abnUpload } from "../../redux/slices/masterDataSlice";
import { fileVaildation } from "@config/app-config";
import { roleIds } from "../../_helpers/constants";
import { useDispatch } from "react-redux";
import { useLocation } from "react-router-dom";

export const FileUploadforABN = ({ open, setOpen, reasonForReject, acceptMsg }) => {
  ...
  const handleFileChange = (e) => {
    ...
    const allowedFileType = [".xlsx", ".xls", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"];

    const fileSizeLimit = 10 * 1024 * 1024;
    if (selectedFile && selectedFile.size <= fileSizeLimit && allowedFileType.includes(selectedFile.type)) {
      ...
      setSelectedFileName("");
      setError("Invalid file. Please select a XLSX/XLS file within 10MB.");
  ...
  const handleDrop = (event) => {
    ...
    if (selectedFile && selectedFile.size <= fileSizeLimit && allowedFileType.includes(selectedFile.type)) {
      ...
      setError("Invalid file. Please drop a XLSX/XLS file within 10MB.");
    }
  };
  ...
  const updateFile = async () => {
    ...
    formData.append("abnRequiredMessage", acceptMsg);
    formData.append("abnNotRequiredMessage", reasonForReject);
    // dispatch(abnUpload(formData));

    setIsLoading(true);

    try {
      const response = await dispatch(abnUpload(formData));
    ...
    <>
      ...
      >
        <Box className="commonModal__wrapper--dialog">
          ...
          <DialogTitle> Add ABN </DialogTitle>
